...
class PostSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // company random
        $company_id = DB::table('users')->where('user_type', 'company')->pluck('id');
        $company_array = $company_id->toArray();

        // job random
        $job_id = DB::table('job_title')->where('id', '<>', '1')->pluck('id');
        $job_array = $job_id->toArray();
        ...
        for ($i = 0; $i < 50; $i++) {
            $title = $job_array[rand(1, count($job_array)-1)];
            ...
            $array_insert[] = [
                ...
                'responsabilities' => $responsibilties,
